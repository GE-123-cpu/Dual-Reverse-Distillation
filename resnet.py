...
class BN_layer(nn.Module):
    def __init__(self,
        ...
        self.conv2 = conv3x3(256 * block.expansion, 512 * block.expansion, 2)
        self.bn2 = norm_layer(512 * block.expansion)

        self.conv3 = conv3x3(768 * block.expansion, 256 * block.expansion, 1)
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        # See note [TorchScript super()]
        #x = self.cbam(x)
        x1 = self.relu(self.bn1(self.conv1(x[0])))
        x11 = torch.cat([x1, x[1]], dim=1)
        x11 = self.relu(self.bn2(self.conv2(x11)))
        output = torch.cat([x11, x[2]], dim=1)
